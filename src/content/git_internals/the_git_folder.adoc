...
* Usually it does not contain a commit link:#_sha1[SHA-1], but contains the path to a file (of the name of the current branch) in the link:#_refs_directory[refs directory] which stores the last commit's SHA-1 hash in that branch.
...
* The link:#_head_file[`HEAD` file] references a file (of the name of link:#_new_branch_name[the branch that is currently checked out]) from the `heads` directory in this (`refs`) directory.

==== The `packed-refs` File

* One file is created per branch and tag in the link:#_refs_directory[`refs` directory].
