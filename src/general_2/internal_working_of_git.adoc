...
A new directory `e6` has been added to `./.git/objects` and it contains one file.
...
Now as mentioned in the previous paragraph, Git stores the contents of the file and other details in the hash. To check that out, some content to the file needs to be added. The file will then have to be added again (this will produce another hash).
...
From the above picture it can be noticed that two new directories `./.git/objects/1b` and `./.git/objects/d5` were created. Also, after commiting the file, Git gives the first 7 characters of the SHA1 for that commit.
...
* `tree 1b2190cdc2801ec3df6505dc351dee878ac7f2fc` is the SHA1 hash of the tree commit generated (remember that two files were generated in `./.git/objects` on committing the file). The tree is the [snapshot] of the current state of the local repository. The contents of the tree file will be looked into soon.
* Parent commit SHA1 hash (not present here - will be looked into in the `Adding another file (parent commit)` section below.)
...
The format of each line is the same. Have a look at `100644 blob 1a3851c172420a2198cf8ca6f2b776589d955cc5 master_file_1.txt`:

* `100644` is the mode, which is some sort of unix-style file permission and file type checking system.
* `blob` is the file type. (It can be a `tree` object as well.)
...
A new line `parent d5b8f77ce1dc1a37b29885026055c8656c3e0b65`is seen. Remember, this is the hash of the previous commit. So Git is creating a graph. (A picture is provided below.) Also, the [`HEAD`] will now automatically point to this (latest) commit rather than the parent (previous) commit as it was doing before. To verify, have a look at where the `HEAD` is pointing.
...
Now have a look at the contents of the tree file (with SHA `f6a65` as seen in the above picture).
...
On comparing the SHA1 hashes of `master_picture_1.png` in the latest (`6d2d2`) and previous (`27666`) tree, we can make out that they are *different*. So Git has created two different blobs (`ca893` and `1f7af`) for the same file, even when they have a very small difference.
...
Now the size of `./.git/objects/pack` in the above picture is 1380 kb and a lot of the files in `./.git/objects` have disappeared, except for `./.git/objects/e6`.
...
+
Have a look at the contents of both the files using the plumbing command `git verify-pack -v path/to/pack/file/<file_name>.pack`. (Works with `.idx` as well.)
+
...
+
On running the aggressive garbage collection, Git got rid of all the files [.underline]#that were related to commits# and added them to the pack file. 
